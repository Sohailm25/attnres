# ABOUTME: Audits how the one-token tool-breakage surface covers saved factual route modes.
# ABOUTME: Reuses existing oracle, route-mode, and tool-breakage artifacts instead of launching another model run.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--factual-route-mode-path",
        default=("results/block_structure/20260318-gemma2-factual-route-modes-v1.json"),
    )
    ...
    parser.add_argument(
        ...
        default="tool_breakage_factual_recall_v4",
    )
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "20260318-gemma2-tool-breakage-one-token-pilot-v4/summary.json"
        ),
    )
    parser.add_argument(
        ...
        default=(
            "results/tool_breakage/"
            "20260318-gemma2-tool-breakage-one-token-confirm-v4/summary.json"
        ),
    )
...
def main() -> int:
    ...
    route_mode_summary = json.loads(Path(args.factual_route_mode_path).read_text())
    ...
    bridge_summary = build_tool_breakage_factual_bridge_summary(
        ...
        cluster_count=int(route_mode_summary["cluster_count"]),
    )
    coverage_summary = build_tool_breakage_route_mode_coverage_summary(
    ...
    payload = {
        ...
        "factual_route_mode_path": str(Path(args.factual_route_mode_path)),

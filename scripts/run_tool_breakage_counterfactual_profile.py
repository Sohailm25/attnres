# ABOUTME: Builds a saved-artifact tag profile for Gemma tool-breakage counterfactual runs.
# ABOUTME: Reuses prompt checkpoints to summarize donor-arm results by family and route mode.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--collection-id",
        default="tool_breakage_factual_recall_v5",
    )
    parser.add_argument("--split", default="confirm")
    parser.add_argument("--counterfactual-summary-path", required=True)
    parser.add_argument("--prompt-checkpoint-dir", required=True)
    parser.add_argument(
        "--group-tag-prefixes",
        nargs="+",
        default=["subcategory_", "route_mode_"],
    )
...
def main() -> int:
    ...
    summary = build_tool_breakage_counterfactual_tag_profile_summary(
        counterfactual_prompt_results=counterfactual_prompt_results,
        prompt_tags_by_id=prompt_tags_by_id,
        group_tag_prefixes=tuple(args.group_tag_prefixes),
    )
    summary.update(
        {
